            ...
            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        'Bot není ve voice kanále.  '
                        'Use %sjoin k přopojení do voice kanálu.' % self.config.command_prefix)
...
#######################################################################################################################
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg = "__**Dostupné příkazy**__\n```"
            commands = []
            ...
            helpmsg += "``` Můžeš také použít `{}help [příkaz]` pro více informací o příkazu.".format(self.config.command_prefix)
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        for user in user_mentions.copy():
            if user.id == self.config.owner_id:
                print("[Commands:Blacklist] Majitel nemůže jít do BlackListu.")
        ...
        if option in ['+', 'add']:
            ...
            return Response(
                '%s byl(a) přidán do blacklistu' % (len(self.blacklist) - old_len),
                reply=True, delete_after=10
            )

        else:
            if self.blacklist.isdisjoint(user.id for user in user_mentions):
                return Response('Žádný z těchto uživatelů není na blacklistu.', reply=True, delete_after=10)
                ...
                return Response(
                    '%s byl(a) odebrán z blacklistu' % (old_len - len(self.blacklist)),
                    reply=True, delete_after=10
                )
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                "dosáhl si limitu skladeb ve frontě" % permissions.max_songs, expire_in=30
            )
        ...
        if not info:
            raise exceptions.CommandError(
                "Tohle video nemůže být přehráno. Zkus {}stream příkaz.".format(self.config.command_prefix),
                expire_in=30
            )
        ...
        if 'entries' in info:
            ...
            if not permissions.allow_playlists and ':search' in info['extractor'] and len(info['entries']) > 1:
                raise exceptions.PermissionsError("Nejsi oprávněn požádat o playlist", expire_in=30)
            ...
            if not listlen - drop_count:
                raise exceptions.CommandError(
                    "žádné další skladby nebyly přidány, Všechny skladby přesáhly maximální délky (%ss)" % permissions.max_song_length,
                    expire_in=30
                )
